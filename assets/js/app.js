...
async function loadAffiliateLinks(){
  try{
    const res = await fetch('/data/affiliate-links.json');
    const data = await res.json();
    (data.tools||[]).forEach(t=>{
      affiliateTools[t.category] = affiliateTools[t.category] || [];
      affiliateTools[t.category].push(t);
    });
  }catch(e){ console.warn('Affiliate properties file not loaded', e); }
}
...
function renderToolLibrary(){
  ...
  target.innerHTML = all.map(t=>`<div class="tool-card"><span class="eyebrow">${t.category}</span><h3><a class="tool-name" data-tool-id="${t.id}" rel="sponsored nofollow noopener" href="${preferredToolUrl(t)}" target="_blank">${t.name}</a></h3><p>${t.best_for||''}</p><small>Rating: ${t.rating||'TBD'}/5</small>${t.free_trial_url?`<br><a class="free-trial-link" href="${preferredTrialUrl(t)}" target="_blank" rel="sponsored nofollow noopener">Start free trial</a>`:''}</div>`).join('');
}
